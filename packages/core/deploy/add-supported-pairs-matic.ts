                ...
                if (tokenArray[i].isQuote) {
                        for (let j = 0; j < tokenArray.length; j++) {
                                if (tokenArray[i] != tokenArray[j]) {
                                        let isSupported = await manager.isPairSupported(
                                                tokenArray[i].address,
                                                tokenArray[j].address
                                        );
                                        if (isSupported) {
                                                console.log(
                                                        `already supported pair: ${tokenArray[i].name} => ${tokenArray[j].name}`
                                                );
                                                continue;
                                        }
                                        console.log(
                                                `add supported pair: ${tokenArray[i].name} => ${tokenArray[j].name}`
                                        );
                                        await manager.addSupportedPair(
                                                tokenArray[i].address,
                                                tokenArray[j].address
                                        );
                                }
