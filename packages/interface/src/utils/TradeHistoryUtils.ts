...
export class TradeHistoryUtils {
  ...
  totalSpent = (positionTrades: PositionTrades) =>
    positionTrades.trades.reduce(
      (total, trade) => (this.isBuy(trade) ? (total += +trade.amount0) : total),
      0
    );
  ...
  buyTrades = (positionTrades: PositionTrades) =>
    positionTrades.trades.reduce(
      (total, trade) => (this.isBuy(trade) ? ++total : total),
      0
    );
  ...
  timeEnd = (positionTrades: PositionTrades) => {
    return toDateTimeStr(
      positionTrades.trades[positionTrades.trades.length - 1].tradeTime
    );
  };
  ...
  positionAmount = (positionTrades: PositionTrades) => {
    return formatAmount(
      Moralis.Units.FromWei(
        this.totalBought(positionTrades),
        this.tradeToken(positionTrades).decimals
      ),
      8
    );
  };
  ...
  profit = (positionTrades: PositionTrades) => {
    ...
    return Moralis.Units.FromWei(
      _profit,
      this.mainToken(positionTrades).decimals
    );
  };
  ...
  avePriceBought = (positionTrades: PositionTrades) => {
    let value =
      this.totalSpent(positionTrades) / this.buyTrades(positionTrades);
    let fromWei = Moralis.Units.FromWei(
      value,
      this.mainToken(positionTrades).decimals
    );
  ...
  avePriceSold = (positionTrades: PositionTrades) => {
    let value =
      this.totalSold(positionTrades) / this.sellTrades(positionTrades);
    let fromWei = Moralis.Units.FromWei(
      value,
      this.mainToken(positionTrades).decimals
    );
  ...
  tradeAmount = (trade: HistoryTrade) => {
    ...
    return formatAmount(
      Moralis.Units.FromWei(trade.amount1, token1.decimals),
      6
    );
  };
  ...
  price = (trade: HistoryTrade) => {
    ...
    if (token0.decimals !== token1.decimals) {
      ...
      if (deciDifference > 0)
        _price = _price.divide(
          new bigDecimal(Math.pow(10, deciDifference)),
          18
        );
      else
  ...
  calcPrice = (trade: HistoryTrade, precision: number) => {
    return new bigDecimal(trade.amount0).divide(
