    ...
    return (
        <div className={`z-50 ${props.className} w-full`}>
            <Listbox value={selected || {}} onChange={onSelect}>
                <div className={`relative mt-1 w-full flex flex-row justify-between ${filteredTokens&& filteredTokens.length>0 ?'':'items-center'}`}>
                    ...
                    <div>
                        {props.input()}
                            { filteredTokens.map(t=>{
                                    return(<Listbox.Option key={t.name} value={t} as={Fragment}>
                                        {({ active, selected }) => (
                                            <li
                                            className={`${
