    ...
    <div className="p-4 rounded-lg bg-secondary">
      ...
      <div className="flex flex-row items-center justify-between">
        ...
        <input
          ...
          value={stopLoss}
          onChange={(e) => {
            dispatch(setStopLoss(e.target.value));
          }}
        />
      </div>
      ...
      <button
        ...
        className={`bg-blue-500 ${
          isValid ? "hover:bg-blue-700" : "cursor-default"
    ...
    (
      ...
      ${
        selected
          ...
          : "bg-white text-black dark:bg-black-type1 dark:text-white"
      } 
      rounded-t-lg p-2
      `}
      >
  ...
  return (
    <Tab.Group>
      <Tab.List className={"bg-white dark:bg-black-type1"}>
        <Tab as={Fragment}>{selectTab("Summary")}</Tab>
        <Tab as={Fragment}>{selectTab("History")}</Tab>
        <Tab as={Fragment}>{selectTab("Update")}</Tab>
      </Tab.List>
      <Tab.Panels>
        <Tab.Panel>
          <DroidView {...props} />
...
export function DroidTransactions(props: DroidProps) {
  return <div>droidtransactions</div>;
}
